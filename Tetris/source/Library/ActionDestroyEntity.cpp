...
namespace Library
{
	...
	*/
	ActionDestroyEntity::ActionDestroyEntity() : mEntity()
	{
		ExternalAttribute(Entity, Datum::STRING, 1, &mEntity);
		Populate();
	}
	...
	*/
	const std::string& ActionDestroyEntity::GetEntityName()
	{
		return mEntity;
	}
	...
	*/
	void ActionDestroyEntity::SetEntityName(const std::string& key)
	{
		mEntity = key;
	}
